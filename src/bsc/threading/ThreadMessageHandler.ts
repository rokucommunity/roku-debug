...
export class ThreadMessageHandler<T, TRequestName = MethodNames<T>> {
    constructor(
        options: {
            name?: string;
    ...
    private onResponse<T, R = WorkerResponse<T>>(id: number): Promise<R> {
        ...
        this.emitter.once(`response-${id}`, (response) => {
            deferred.resolve(response as R);
    ...
    private errorToObject(error: Error) {
        return {
            ...
            stack: error.stack,
            cause: (error.cause as any)?.message && (error.cause as any)?.stack ? this.errorToObject(error.cause as unknown as Error) : error.cause
        };
    }
